...
export async function fetchCourse(subject: string, catalogNo: String): Promise<Courses | null> {
  try {
    const response = await fetch(`http://127.0.0.1:8000/course/${subject}/${catalogNo}`);
    if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
    return await response.json();
  } catch (error) {
    console.error('Error:', error);
