...
export default function AllSubjectsPage() {
  ...
  const numCourses = sum(
    map(allSubjectsData?.items, (subject) => subject.courses_offered || 0),
  );
  const numHeadcount = sum(
    map(allSubjectsData?.items, (subject) => subject.total_headcount || 0),
  );
  ...
  const enrollmentTreeMap = (
    <EnrollemntTreeMap data={treeMapData} onClick={onClickTreeMap} />
  );
  ...
  return (
    <div>
      <div className="row shadow-sm py-3 bg-black rounded-bottom">
        <h2>
          Loaded {numCourses} courses with {numHeadcount} total enrollments
        </h2>
