...
export default function CourseOverviewPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const subjects = await fetchCourse(
        subject || "CPT_S",
        catalogNo || "121",
      );
  ...
  const pagenation = (
    <Pagination
      limitedCourseData={limitedCourseData}
      page={page}
      setPage={setPage}
    />
  );
  ...
  return (
    <div>
        ...
        <Tabs
          tabs={[
            ...
            },
            {
              ...
              component: (
                <InstructorAllocationBarChart
                  data={limitedCourseData.courseData}
                />
              ),
            },
          ]}
      ...
      <div className="row py-3 bg-dark-subtle">
        <h2>
          Offerings (displaying {size(limitedCourseData.courseData?.courses)} /{" "}
          {size(courseData?.courses)})
        </h2>
        {limitedCourseData.courseData && (
          <CourseTable data={limitedCourseData.courseData} />
        )}
      </div>
    </div>
  );
