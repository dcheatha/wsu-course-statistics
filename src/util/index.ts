...
export function CourseDataInstructorLimiter(
  courseData: Courses | null,
  page: number,
): { courseData: Courses | null; pages: string[] } {
  const instructors = uniq(
    map(courseData?.courses, (course) => course.instructor || "Unknown"),
  );
  ...
  const courseDataYears = keys(
    groupBy(courseData?.courses, (courseGrade) => courseGrade.year),
  );

  const groupedCourseData = values(
    groupBy(courseData?.courses, (courseGrade) => courseGrade.year),
  );
  const groupedGradeData = values(
    groupBy(
      courseData?.stats.grades_by_instructor,
      (courseGrade) => courseGrade.year,
    ),
  );
...
export function groupGrades(data: Courses | null) {
  return chain(data?.stats.grades_by_instructor)
    ...
    .mapValues((yearSemesterGroup) =>
      chain(yearSemesterGroup)
        .groupBy("instructor")
        .map((data) => {
          // @ts-ignore
          return chain(data)
